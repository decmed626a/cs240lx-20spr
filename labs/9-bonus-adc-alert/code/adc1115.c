...
void notmain(void) {
    ...
    enum { conversion_reg = 0x00, config_reg = 0x01,
	       lo_thresh_reg = 0x02, hi_thresh_reg = 0x03};
...
#   define PGA_V(x) (((x)>>9)&0b111)
	...
	config_val = 0b100 << 12 |  // Set AIN0 and AIN1 for diff input
				 ...
				 0b1 << 3 |		// Set active high polarity
				 0b0 << 2 |		// Set latching comparator
				 0b00;			// Enable comparator assert after one conversion

	adc_write16(dev_addr, config_reg, config_val); 
    adc_write16(dev_addr, lo_thresh_reg, 0x1000);
    adc_write16(dev_addr, hi_thresh_reg, 0x4000);
